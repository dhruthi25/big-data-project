...
def readStream(rdd):
  ...
  if not rdd.isEmpty():
    ...
    stages+=[stopremove]
    Hv=HashingTF(inputCol='stop_tokens',outputCol='token_features')
    stages+=[Hv]
    ...
    load_clusteringmodel=pickle.load(open(filename,'rb'))
    outputarr=load_clusteringmodel.predict(clean_data_np)
    

    output=load_clusteringmodel.fit(clean_data_np)
    #print('worked?')
    #with Kmeans
    ...
    mbk_means_labels = load_clusteringmodel.labels_
    mbk_means_cluster_centers = load_clusteringmodel.cluster_centers_
